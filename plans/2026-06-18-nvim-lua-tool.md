...
Code review (Stage 2): the only new conditional logic is the `case "nvim_lua"` throw in `create-tool.ts:155` when `context.luaExecutor` is undefined. This is a defensive invariant — `getToolSpecs` capability filtering ("nvim") prevents the tool from ever being offered without a `luaExecutor` — so it is unreachable in practice and intentionally left untested. The realistic capability-filter guard is covered by `toolManager.test.ts`, and the tool's execute/validate logic by `nvimLua.test.ts`. No code change required.
...
**Status: COMPLETE.** Created `node/capabilities/nvim-lua-executor.ts` (`NvimLuaExecutor implements LuaExecutor`, wrapping `nvim.call("nvim_exec_lua", [code, []])` and converting a `null` msgpack result to `undefined`). Exported `LuaExecutor` from `node/core/src/index.ts`. In `node/environment.ts`: added `luaExecutor?` to the `Environment` interface, `createLocalEnvironment` constructs a `NvimLuaExecutor`, provides it, and adds `"nvim"` to `availableCapabilities`; `createDockerEnvironment` omits it. In `node/chat/thread.ts`, threaded `luaExecutor` into the `ThreadCore` context at both construction sites (new-thread ~line 302, clone ~line 530) via conditional spread (only when defined). Added integration tests in `node/tools/nvimLua.test.ts`: `return 1 + 2` → `3`; a side effect (`vim.g.magenta_test = 42`) observable via the driver's nvim; and `error('boom')` surfaced as a `status: "error"` result. Updated 3 `thread.test.ts` snapshots (Tool Definitions count 7→8 now that `nvim_lua` is offered to local root threads). Full `npx tsgo -b`, `npx biome check .`, and `npx vitest run node/tools/nvimLua.test.ts` pass. The full suite is green except two pre-existing flakes unrelated to this change: `spawn-subagents` "hierarchy view" (passes in isolation, on both clean tree and with changes — parallel-execution flake) and `docker-sync` (requires a live docker container). Code review (Stage 3): added a `nvim_lua handles a nil return value` integration test (`code: "local x = 1"`, no return) asserting the "Executed successfully, no return value." message, covering the `result ?? undefined` nil-to-undefined conversion in `nvim-lua-executor.ts`.
