...
- [ ] extend the `thinking` type in `node/core/src/provider-options.ts` and `node/options.ts` to include an optional `effort` field with values `"low" | "medium" | "high" | "xhigh" | "max"`
...
- [ ] update the runtime validator in `node/options.ts` (the `"thinking" in p` block around line 351) to accept and validate the `effort` string
...
- [ ] update `createNativeStreamParameters` in `node/core/src/providers/anthropic-agent.ts` to set `output_config.effort` when `thinking.effort` is provided
...
- [ ] for non-adaptive models, assert effort is ignored and a warning is logged
...
- [ ] apply the same logic in `forceToolUse` in `node/core/src/providers/anthropic.ts` so single-call tool requests honor the same effort setting
...
- [ ] optional polish: surface the effort level in the sidebar title in `node/sidebar.ts`
...
- [ ] run `TEST_MODE=sandbox npx vitest run node/providers/anthropic-agent.test.ts node/options.test.ts` and iterate until green
