...
**Status: DONE.** `FileState` gained `originalContent` (set in `getOrLoadFile` to the read content; `""` for `newfile`). `Executor.execute()` builds an `originalContents` map alongside `fileContents` and returns it on `ScriptResult`. `runScript` surfaces `previousContent` on each `EdlResultData.mutations` entry. Added unit tests in `node/core/src/edl/index.test.ts` (edited existing file reports original content; new file reports `""`). Full typecheck, `node/core` tests, and biome all pass; one edl snapshot updated.
